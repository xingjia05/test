    ...
    public function __construct($host, $port, $path) {
        ...
        while (true) {
            ...
            if ($client) {
                ...
                var_dump($buf);
                //解析客户端发送过来的协议
                $classRet = preg_match("/Rpc-Class:\s(.*);/is", $buf, $class);
                $methodRet = preg_match("/Rpc-Method:\s(.*);/is", $buf, $method);
                $paramsRet = preg_match("/Rpc-Params:\s(.*);/is", $buf, $params);
                var_dump($classRet, $methodRet, $paramsRet);
